...
grid = Grid([-3, 0.5, 3], [-3, 0.5, 3], color = lightgray)
grid.draw()
...
lagrange.setup()
graph = Graph(lagrange, color = blue)
graph.draw()
# now restore the unclipped state
restore()

for p in points:
    point = Point(p, fillcolor = lightred)
    point.fill()
    point.draw()
...
grid = Grid([-3, 0.5, 3], [-3, 0.5, 3], lightgray)
grid.draw()
...
for p in points:
    ...
    tangent = Graph(f, domain=[p[0]-dx, p[0]+dx], color = red)
    tangent.draw()

    point = Point(p, size = 3, fillcolor = lightred)
    point.setfillcolor(lightred)
    point.fill()
    point.draw()

label = Label("Hermite", [-3, 3], offset = [0,2])
label.draw()
