...
beginfigure("3_2_signchartg2", width, height)
...
label = Label(r"CCD", [-1.5, -0.5], alignment = "ct", offset = [0,-2])
label.draw()
...
label = Label(r"$\frac{2}{b}$", [a, -0.5], alignment = "ct", offset = [0,-2])
label.draw()
...
label = Label(r"CCU", [1.5, -0.5], alignment = "ct", offset = [0,-2])
label.draw()


label = Label(r"$g''(x) = abe^{-bx}(bx-2)$", [0, 3], alignment = "cb", offset = [0,2])
label.draw()
